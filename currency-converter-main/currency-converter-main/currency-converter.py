...
def get_rates(currency: str):
    global window
    global filepath
    window['status_text'].update("Exchange oranları için bekleniyor", text_color='white')
    window.refresh()
    try:
        response = requests.get(f'https://open.er-api.com/v6/latest/{currency}').json()
    except requests.ConnectionError:
        raise Exception("No internet connection hatası")
    except Exception as exception:
        raise Exception(f"Dönüşüm oranlarında hata {exception}")
    try:
...
filepath = 'rates.json'
window = sg.Window("Döviz Excange", layout)
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        if values['input'] == '':
            show_error("Lütfen bir miktar giriniz")
